...
class Notebook(ObjectModel):
    ...
    def _prepare_save_data(self) -> Dict[str, Any]:
        ...
        for key in ("org_id", "user_id"):
            val = data.get(key)
            if val is not None and isinstance(val, str) and val:
                data[key] = ensure_record_id(val)
        org_ids_val = data.get("org_ids")
        if org_ids_val and isinstance(org_ids_val, list):
            data["org_ids"] = [ensure_record_id(oid) if isinstance(oid, str) else oid for oid in org_ids_val]
        return data
...
class Source(ObjectModel):
    ...
    @field_validator("id", "command", "user_id", "org_id", mode="before")
    @classmethod
    def parse_record_ref_fields(cls, value):
        """Parse fields that may arrive as RecordID objects from database"""
        if value is None:
            return None
    ...
    def _prepare_save_data(self) -> dict:
        ...
        data = super()._prepare_save_data()
        for key in ("org_id", "user_id", "command"):
            val = data.get(key)
            if val is not None and isinstance(val, str) and val:
                data[key] = ensure_record_id(val)
        return data
...
async def _filter_search_results(
    ...
    """Post-filter search results by org_ids / user_id ownership."""
    if not results or (not org_ids and not user_id):
        return results
    ...
    allowed_note_ids: set = set()

    if source_ids:
        record_ids = [ensure_record_id(sid) for sid in source_ids]
        if org_ids and user_id:
            org_rids = [ensure_record_id(oid) for oid in org_ids]
            rows = await repo_query(
                "SELECT id FROM source WHERE id IN $ids AND (org_id IN $org_ids OR user_id = $user_id OR (user_id IS NONE AND org_id IS NONE) OR (user_id IS NULL AND org_id IS NULL))",
                {"ids": record_ids, "org_ids": org_rids, "user_id": ensure_record_id(user_id)},
            )
        elif org_ids:
            org_rids = [ensure_record_id(oid) for oid in org_ids]
            rows = await repo_query(
                "SELECT id FROM source WHERE id IN $ids AND org_id IN $org_ids",
                {"ids": record_ids, "org_ids": org_rids},
            )
        else:
            rows = await repo_query(
                "SELECT id FROM source WHERE id IN $ids AND (user_id = $user_id OR user_id IS NONE OR user_id IS NULL) AND (org_id IS NONE OR org_id IS NULL)",
                {"ids": record_ids, "user_id": ensure_record_id(user_id)},
            )
        allowed_source_ids = {str(row["id"]) for row in (rows or [])}

    if note_ids:
        record_ids = [ensure_record_id(nid) for nid in note_ids]
        if org_ids and user_id:
            org_rids = [ensure_record_id(oid) for oid in org_ids]
            rows = await repo_query(
                "SELECT DISTINCT in as note_id FROM artifact WHERE in IN $ids AND (out.org_id IN $org_ids OR out.user_id = $user_id OR (out.user_id IS NONE AND out.org_id IS NONE))",
                {"ids": record_ids, "org_ids": org_rids, "user_id": ensure_record_id(user_id)},
            )
        elif org_ids:
            org_rids = [ensure_record_id(oid) for oid in org_ids]
            rows = await repo_query(
                "SELECT DISTINCT in as note_id FROM artifact WHERE in IN $ids AND out.org_id IN $org_ids",
                {"ids": record_ids, "org_ids": org_rids},
            )
        else:
            rows = await repo_query(
                "SELECT DISTINCT in as note_id FROM artifact WHERE in IN $ids AND (out.user_id = $user_id OR out.user_id IS NONE) AND (out.org_id IS NONE OR out.org_id IS NULL)",
                {"ids": record_ids, "user_id": ensure_record_id(user_id)},
            )
        allowed_note_ids = {str(row["note_id"]) for row in (rows or [])}

    filtered = []
    for r in results:
        pid = str(r.get("parent_id", ""))
        if pid.startswith("source:") and pid in allowed_source_ids:
            filtered.append(r)
        elif pid.startswith("note:") and pid in allowed_note_ids:
            filtered.append(r)
    return filtered
...
async def text_search(
    ...
    needs_filter = bool(org_ids or user_id)
    fetch_count = results * _SEARCH_OVERFETCH_MULTIPLIER if needs_filter else results
    try:
        search_results = await repo_query(
            ...
            """,
            {"keyword": keyword, "results": fetch_count, "source": source, "note": note},
        )
        if needs_filter:
            search_results = await _filter_search_results(
                search_results, org_ids, user_id
            )
