...
async def _get_user_org_ids(current_user: Optional[dict]) -> list:
...
async def get_sources(
    ...
    """Get sources with pagination and sorting support, filtered by org/user scope."""
    try:
        ...
        # Always include the public org so all users can see public notebook sources
        if user_id:
            try:
                public_org_id = await get_public_org_id()
                if public_org_id and public_org_id not in org_ids:
                    org_ids = org_ids + [public_org_id]
            except Exception as e:
                logger.warning(f"Failed to get public org for sources query: {e}")
        ...
        if notebook_id:
            ...
            query = f"""
                ...
                WHERE org_id IN $org_ids
                   OR user_id = $user_id
                   OR (user_id IS NONE AND org_id IS NONE)
                   OR (user_id IS NULL AND org_id IS NULL)
                   OR id IN (
                       SELECT VALUE in FROM reference
                       WHERE out IN (
                           SELECT VALUE id FROM notebook
                           WHERE (org_ids IS NOT NONE AND org_ids IS NOT NULL AND array::intersect(org_ids, $org_ids) != [])
                              OR user_id = $user_id
                       )
                   )
                {order_clause}
            ...
            result = await repo_query(
                query,
                {"org_ids": org_rids, "user_id": ensure_record_id(user_id), "limit": limit, "offset": offset},
            )
        elif user_id:
            query = f"""
                SELECT id, asset, created, title, updated, topics, command,
                ...
                WHERE user_id = $user_id
                   ...
                   OR id IN (
                       SELECT VALUE in FROM reference
                       WHERE out IN (
                           SELECT VALUE id FROM notebook WHERE user_id = $user_id
                       )
                   )
                {order_clause}
                LIMIT $limit START $offset
            ...
            result = await repo_query(
                query,
                {"user_id": ensure_record_id(user_id), "limit": limit, "offset": offset},
            )
        else:
            query = f"""
                SELECT id, asset, created, title, updated, topics, command,
...
async def retry_source_processing(source_id: str):
    ...
    try:
        ...
        try:
            ...
            source.command = f"command:{command_id}"
            await source.save()
